...
DWORD WINAPI CClient::_WorkerThread(LPVOID lpParam)
{
	...
	//TRACE("向服务器发送信息成功: %s\n", szTemp);
	pClient->ShowMessage(L"向服务器发送信息成功: %s", szTemp);
...
bool CClient::ConnetToServer( SOCKET *pSocket, CString strServer, int nPort )
{
	...
	*pSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	...
	char szServer[512] = { 0 };
	WideCharToMultiByte(CP_ACP, 0, strServer.GetString(), strServer.GetLength(), szServer, 512, NULL, 0);
	// 生成地址信息
	Server = gethostbyname(szServer);
	if (Server == NULL) 
	{
		closesocket(*pSocket);
		TRACE("错误：无效的服务器地址.\n");
